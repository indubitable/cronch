...
public partial class ExecutionPersistenceService(ILogger<ExecutionPersistenceService> _logger, IConfiguration _configuration)
{
	...
	public virtual IEnumerable<ExecutionModel> GetRecentExecutions(int maxCount, Guid? jobIdFilter, ExecutionStatus? executionStatusFilter)
	{
		using var db = new SqliteConnection(_connectionString);
		db.Open();
		var queryParams = new
		{
			jobId = jobIdFilter.HasValue ? jobIdFilter.Value.ToString("D").ToUpperInvariant() : string.Empty,
			executionStatus = executionStatusFilter.HasValue ? executionStatusFilter.Value.ToString() : string.Empty,
			limit = maxCount
		};
		var jobIdQuery = jobIdFilter.HasValue ? "AND JobId = @jobId" : string.Empty;
		var executionStatusQuery = executionStatusFilter.HasValue ? "AND Status = @executionStatus" : string.Empty;

		IEnumerable<dynamic> rawData = db.Query(@$"SELECT * FROM Execution WHERE 1=1 {jobIdQuery} {executionStatusQuery} ORDER BY StartedOn DESC LIMIT @limit", queryParams);
